...
def main(filename):
    ...
    # import the diagnostic report
    diagnostic_report = list()
    with open(file_path) as input_file:
        for line in input_file:
            diagnostic_report.append(line.strip())
    num_bits = len(diagnostic_report[0])
    # find the oxygen generator rate
    oxygen_candidates = diagnostic_report
    for digit in range(num_bits):
        zero_in_position = list()
        one_in_position = list()
        for row in oxygen_candidates:
            if row[digit] == '0':
                zero_in_position.append(row)
            else:
                one_in_position.append(row)
        if len(zero_in_position) > len(one_in_position):
            oxygen_candidates = zero_in_position
        else:
            oxygen_candidates = one_in_position
        if len(oxygen_candidates) == 1:
            break
    # find the co2 scrubber rate
    co2_candidates = diagnostic_report
    for digit in range(num_bits):
        zero_in_position = list()
        one_in_position = list()
        for row in co2_candidates:
            if row[digit] == '0':
                zero_in_position.append(row)
            else:
                one_in_position.append(row)
        if len(zero_in_position) <= len(one_in_position):
            co2_candidates = zero_in_position
        else:
            co2_candidates = one_in_position

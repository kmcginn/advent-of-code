...
class Line:
    def __init__(self, input_string):
        ...
        ax, ay = a.strip().split(",")
        bx, by = b.strip().split(",")
        self.start = Point(ax, ay)
        self.end = Point(bx, by)
...
def main(filename):
    ...
    with open(file_path) as input_file:
        for input_line in input_file:
            next_line = Line(input_line)
            lines.append(next_line)
            # keep track of the highest x and y values
            if next_line.start.x > highest_x:
                highest_x = next_line.start.x
            if next_line.end.x > highest_x:
                highest_x = next_line.end.x
            if next_line.start.y > highest_y:
                highest_y = next_line.start.y
            if next_line.end.y > highest_y:
                highest_y = next_line.end.y
    # create the map from the highest values
    # for each line, mark the points in the map that it covers
    # go through all the points, count the number with a value > 2

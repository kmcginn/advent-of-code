...
def main():
    ...
    with open(file_path) as input_file:
        datapoints = list(map(int,input_file.read().split()))
    for i in range(len(datapoints) - 2):
        curr_window_sum = sum(map(int, datapoints[i:i+3]))
        if prev_depth and curr_window_sum > prev_depth:
            depth_increases += 1
        prev_depth = curr_window_sum
